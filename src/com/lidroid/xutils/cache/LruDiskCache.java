...
public final class LruDiskCache implements Closeable {
	...
	private synchronized void completeEdit(Editor editor, boolean success) throws IOException {
		...
		if (entry.readable | success) {
			entry.readable = true;
			journalWriter.write(CLEAN + " " + entry.diskKey + " " + EXPIRY_PREFIX + entry.expiryTimestamp
					+ entry.getLengths() + '\n');
			if (success) {
	...
	public final class Editor {
		 ...
		 * encounters errors
		 * when writing to the filesystem, this edit will be aborted when {@link #commit} is called. The returned output
	...
	private class StrictLineReader implements Closeable {
		 ...
		 *             if {@code in} or {@code charset} is null.
		 * @throws IllegalArgumentException
		 *             if {@code capacity} is negative or zero
