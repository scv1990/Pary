...
public class ForeignLazyLoader<T> {
	...
	public List<T> getAllFromDb() throws DbException {
		...
		if (table != null) {
			entities = table.db.findAll(Selector.from(foreignColumn.getForeignEntityType()).where(
	...
	public T getFirstFromDb() throws DbException {
		...
		Table table = foreignColumn.getTable();
		if (table != null) {
			entity = table.db.findFirst(Selector.from(foreignColumn.getForeignEntityType()).where(
					foreignColumn.getForeignColumnName(), "=", columnValue));
		}
		return entity;
	}
