...
public class FinderLazyLoader<T> {
	...
	public List<T> getAllFromDb() throws DbException {
		...
		if (table != null) {
			entities = table.db.findAll(Selector.from(finderColumn.getTargetEntityType()).where(
	...
	public T getFirstFromDb() throws DbException {
		...
		Table table = finderColumn.getTable();
		if (table != null) {
			entity = table.db.findFirst(Selector.from(finderColumn.getTargetEntityType()).where(
					finderColumn.getTargetColumnName(), "=", finderValue));
		}
		return entity;
	}
